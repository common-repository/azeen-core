...
if ( ! class_exists( 'AzeenWP_Demos' ) ) {
	...
	class AzeenWP_Demos {
		...
		public function init() {
			...
			// Demos popup ajax
			add_action( 'wp_ajax_azeen_ajax_get_demo_data', array( $this, 'ajax_demo_data' ) );
			add_action( 'wp_ajax_azeen_ajax_required_plugins_activate', array( $this, 'ajax_required_plugins_activate' ) );

			// Get data to import
			add_action( 'wp_ajax_azeen_ajax_get_import_data', array( $this, 'ajax_get_import_data' ) );

			// Import XML file
			add_action( 'wp_ajax_azeen_ajax_import_xml', array( $this, 'ajax_import_xml' ) );

			// Import customizer settings
			add_action( 'wp_ajax_azeen_ajax_import_theme_settings', array( $this, 'ajax_import_theme_settings' ) );

			// Import widgets
			add_action( 'wp_ajax_azeen_ajax_import_widgets', array( $this, 'ajax_import_widgets' ) );

      // Reset theme mods
			add_action( 'wp_ajax_azeen_ajax_reset_mods', array( $this, 'ajax_reset_mods' ) );

			// After import
			add_action( 'wp_ajax_azeen_after_import', array( $this, 'ajax_after_import' ) );
		...
		public static function scripts( $hook_suffix ) {
			...
			if ( 'appearance_page_azeen-panel-install-demos' == $hook_suffix ) {
				...
				wp_localize_script( 'fwp-demos-js', 'azeenDemos', array(
					'ajaxurl' 					=> admin_url( 'admin-ajax.php' ),
					'demo_data_nonce' 			=> wp_create_nonce( 'get-demo-data' ),
					'azeen_import_data_nonce' 	=> wp_create_nonce( 'azeen_import_data_nonce' ),
					'content_importing_error' 	=> esc_html__( 'There was a problem during the importing process resulting in the following error from your server:', 'azeen-core' ),
					'button_activating' 		=> esc_html__( 'Activating', 'azeen-core' ) . '&hellip;',
					'button_active' 			=> esc_html__( 'Active', 'azeen-core' ),
				) );
		...
		public static function get_demos_data() {
			...
			// Demos url
			$url = 'https://azeendemos.com/wp-content/uploads/demos/';

			$data = array(
        'elementor-blog-demo' => array(
					'categories'        => array( 'Blog', 'Elementor' ),
					'xml_file'     		=> $url . 'elementor-blog-demo/sample-data.xml',
					'theme_settings' 	=> $url . 'elementor-blog-demo/azeen-export.json',
					...
					'required_plugins'  => array(
						'free' => array(
							array(
								'slug'  	=> 'azeen-core',
								'init'  	=> 'azeen-core/azeen-core.php',
								'name'  	=> 'Azeen Core',
							),
							array(
								'slug'  	=> 'elementor',
        ...
        'blog-demo' => array(
					...
					'theme_settings' 	=> $url . 'blog-demo/azeen-export.json',
			...
			// Return
			return apply_filters( 'azeen_demos_data', $data );
	    ...
	    public static function popup() {
	       ...
	       if ( ( 'themes.php' == $pagenow && isset($_GET['page']) && 'azeen-panel-install-demos' == $_GET['page'] ) ) {
              ?>
      ...
      // Get recommended plugins
			...
			<div id="fwp-demo-plugins">
				...
				<h2 class="title"><?php echo sprintf( esc_html__( 'Import the %1$s demo', 'azeen-core' ), esc_attr( $demo ) ); ?></h2>

				<div class="fwp-popup-text">

					<p><?php echo
						sprintf(
							esc_html__( 'Importing demo data allow you to quickly edit everything instead of creating content from scratch. It is recommended uploading sample data on a fresh WordPress install to prevent conflicts with your current content. You can use this plugin to reset your site if needed: %1$sWordpress Database Reset%2$s.', 'azeen-core' ),
							'<a href="'. $plugin_link .'" target="_blank">',
							'</a>'
						); ?></p>

					<div class="fwp-required-plugins-wrap">
						<h3><?php esc_html_e( 'Required Plugins', 'azeen-core' ); ?></h3>
						<p><?php esc_html_e( 'For your site to look exactly like this demo, the plugins below need to be activated.', 'azeen-core' ); ?></p>
            ...
            <?php	if ( isset($recommended) && $recommended != '' )  { ?>
              <h3><?php esc_html_e( 'Recommended Plugins', 'azeen-core' ); ?></h3>
  						<p><?php esc_html_e( 'These plugins are not required for the demo import. However, if you do not install them, some demo features will not be included.', 'azeen-core' ); ?></p>
  						<div class="fwp-required-plugins oe-plugin-installer">
        ...
        <?php if ( !defined('azeen_PRO_CURRENT_VERSION') && $premium['0']['slug'] == 'azeen-pro' ) { ?>
          <div class="fwp-button fwp-plugins-pro">
            <a href="<?php echo esc_url( 'https://azeenwp.com/azeen-pro/'); ?>" target="_blank" >
              <?php esc_html_e( 'Install and activate azeen PRO', 'azeen-core' ); ?>
            </a>
          </div>
        <?php } elseif ( defined('azeen_PRO_CURRENT_VERSION') && !defined('azeen_SLT_PRO') && $premium['0']['slug'] == 'azeen-pro' ) { ?>
          <div class="fwp-button fwp-plugins-pro">
            <a href="<?php echo esc_url( network_admin_url('options-general.php?page=azeen-license-options') ) ?>" >
              <?php esc_html_e( 'Activate azeen PRO license', 'azeen-core' ); ?>
            </a>
          </div>
        <?php } else { ?>
          <div class="fwp-button fwp-plugins-next">
            <a href="#">
              <?php esc_html_e( 'Go to the next step', 'azeen-core' ); ?>
            </a>
          </div>
        <?php } ?>
			...
			<form method="post" id="fwp-demo-import-form">
				...
				<input id="azeen_import_demo" type="hidden" name="azeen_import_demo" value="<?php echo esc_attr( $demo ); ?>" />

				<div class="fwp-demo-import-form-types">

					<h2 class="title"><?php esc_html_e( 'Select what you want to import:', 'azeen-core' ); ?></h2>

					<ul class="fwp-popup-text">
            <li>
							<label for="azeen_reset_mods">
								<input id="azeen_reset_mods" type="checkbox" name="azeen_reset_mods"  />
								<strong><?php esc_html_e( 'Reset theme options', 'azeen-core' ); ?></strong> (<?php esc_html_e( 'Customizer options', 'azeen-core' ); ?>)
							</label>
						</li>
						<li>
							<label for="azeen_import_xml">
								<input id="azeen_import_xml" type="checkbox" name="azeen_import_xml" checked="checked" />
								<strong><?php esc_html_e( 'Import XML Data', 'azeen-core' ); ?></strong> (<?php esc_html_e( 'pages, posts, images, menus, etc...', 'azeen-core' ); ?>)
							</label>
						</li>

						<li>
							<label for="azeen_theme_settings">
								<input id="azeen_theme_settings" type="checkbox" name="azeen_theme_settings" checked="checked" />
								<strong><?php esc_html_e( 'Import Customizer Settings', 'azeen-core' ); ?></strong>
							</label>
						</li>

						<li>
							<label for="azeen_import_widgets">
								<input id="azeen_import_widgets" type="checkbox" name="azeen_import_widgets" checked="checked" />
								<strong><?php esc_html_e( 'Import Widgets', 'azeen-core' ); ?></strong>
							</label>
						</li>
					</ul>
				...
				<?php wp_nonce_field( 'azeen_import_demo_data_nonce', 'azeen_import_demo_data_nonce' ); ?>
				<input type="submit" name="submit" class="fwp-button fwp-import" value="<?php esc_html_e( 'Install this demo', 'azeen-core' ); ?>"  />

			</form>

			<div class="fwp-loader">
				<h2 class="title"><?php esc_html_e( 'The import process could take some time, please be patient', 'azeen-core' ); ?></h2>
			...
			<div class="fwp-last">
				...
				<h3><?php esc_html_e( 'Demo Imported!', 'azeen-core' ); ?></h3>
				<a href="<?php echo esc_url( get_home_url() ); ?>"" target="_blank"><?php esc_html_e( 'See the result', 'azeen-core' ); ?></a>
			</div>
			...
			foreach ( $plugins as $key => $plugin ) {
				...
				if ( ! is_wp_error( $api ) ) { // confirm error free
					...
					if ( file_exists( WP_PLUGIN_DIR . '/' . $plugin['init'] ) && is_plugin_inactive( $plugin['init'] ) ) {
						...
						$button_text 	= esc_html__( 'Activate', 'azeen-core' );

					// Not Installed.
					} elseif ( ! file_exists( WP_PLUGIN_DIR . '/' . $plugin['init'] ) ) {

						$button_classes = 'button install-now';
						$button_text 	= esc_html__( 'Install Now', 'azeen-core' );

					// Active.
					} else {
						$button_classes = 'button disabled';
						$button_text 	= esc_html__( 'Activated', 'azeen-core' );
					} ?>
					...
					<div class="fwp-plugin fwp-clr fwp-plugin-<?php echo $api['slug']; ?>" data-slug="<?php echo $api['slug']; ?>" data-init="<?php echo $api['init']; ?>">
						...
						if ( 'premium' == $return
							...
							<a class="button" href="https://azeenwp.com/<?php echo $api['slug']; ?>/" target="_blank"><?php esc_html_e( 'Get This Addon', 'azeen-core' ); ?></a>
						<?php
						} else { ?>
		...
		public function ajax_required_plugins_activate() {
			...
			if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['init'] ) || ! $_POST['init'] ) {
				wp_send_json_error(
					array(
						'success' => false,
						'message' => __( 'No plugin specified', 'azeen-core' ),
					)
				);
			}
			...
			wp_send_json_success(
				array(
					'success' => true,
					'message' => __( 'Plugin Successfully Activated', 'azeen-core' ),
				)
			);
		...
		public function ajax_get_import_data() {
			check_ajax_referer( 'azeen_import_data_nonce', 'security' );

			echo json_encode(
				array(
          array(
						'input_name' 	=> 'azeen_reset_mods',
						'action' 		=> 'azeen_ajax_reset_mods',
						'method' 		=> 'ajax_reset_mods',
						'loader' 		=> esc_html__( 'Reseting Theme Options', 'azeen-core' )
					),
					array(
						'input_name' 	=> 'azeen_import_xml',
						'action' 		=> 'azeen_ajax_import_xml',
						'method' 		=> 'ajax_import_xml',
						'loader' 		=> esc_html__( 'Importing XML Data', 'azeen-core' )
					),

					array(
						'input_name' 	=> 'azeen_theme_settings',
						'action' 		=> 'azeen_ajax_import_theme_settings',
						'method' 		=> 'ajax_import_theme_settings',
						'loader' 		=> esc_html__( 'Importing Customizer Settings', 'azeen-core' )
					),

					array(
						'input_name' 	=> 'azeen_import_widgets',
						'action' 		=> 'azeen_ajax_import_widgets',
						'method' 		=> 'ajax_import_widgets',
						'loader' 		=> esc_html__( 'Importing Widgets', 'azeen-core' )
					),
				)
			);
    ...
    /**
		...
		public function ajax_reset_mods() {
			if ( ! wp_verify_nonce( $_POST['azeen_import_demo_data_nonce'], 'azeen_import_demo_data_nonce' ) ) {
				die( 'This action was stopped for security purposes.' );
			}
      $save_result = 1;
      // Get the selected demo
			$demo_type 			= $_POST['azeen_reset_mods'];
      ...
      if( $save_result === 1 ){
        ...
        $directory = trailingslashit( $upload_dir['basedir'] ) . 'azeen-files';
        if ( ! is_dir( $directory ) ) {
    			wp_mkdir_p( $directory );
    		}
        if (is_writable($directory) ) {
    			$file = $directory . '/azeen' . date('Y-m-d-h-i-s') . '.json';
      ...
      if ( is_wp_error( $result ) ) {
		...
		public function ajax_import_xml() {
			...
			// Get the selected demo
			$demo_type 			= $_POST['azeen_import_demo'];

			// Get demos data
			$demo 				= azeenWP_Demos::get_demos_data()[ $demo_type ];
		...
		public function ajax_after_import() {
			...
			if ( $_POST['azeen_import_is_xml'] === 'true' ) {
		...
		public function process_xml( $file ) {
			...
			if ( $response === false ) {
				return new WP_Error( 'xml_import_error', __( 'Can not retrieve sample data xml file. Website may be down at the momment please try again later. If you still have issues contact the theme developer for assistance.', 'azeen-core' ) );
			}
			...
			if ( file_exists( $attachment_url ) ) {
				...
				return new WP_Error( 'xml_import_error', __( 'The xml import file could not be accessed. Please try again or contact the theme developer.', 'azeen-core' ) );
			}
		...
		private function import_xml( $file ) {
			...
			if ( ! class_exists( 'WP_Importer' ) ) {
				...
				if ( file_exists( $class_wp_importer ) ) {
					...
					$importer_error = __( 'Can not retrieve class-wp-importer.php', 'azeen-core' );
				}
			}
			...
			if ( ! class_exists( 'WP_Import' ) ) {
				...
				if ( file_exists( $class_wp_import ) ) {
					...
					$importer_error = __( 'Can not retrieve wordpress-importer.php', 'azeen-core' );
				}
			}
			...
			if ( $importer_error ) {
				...
				if ( ! is_file( $file ) ) {
					$importer_error = __( 'Sample data file appears corrupt or can not be accessed.', 'azeen-core' );
					return new WP_Error( 'xml_import_error', $importer_error );
				} else {
