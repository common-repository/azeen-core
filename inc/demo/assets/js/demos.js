( function( $ ) {
	...
	var fwpDemoImport = {
		...
		getDemoData: function( demo_name ) {
			...
			// Get import data
			$.ajax( {
				url: azeenDemos.ajaxurl,
				type: 'get',

				data: {
					action: 'azeen_ajax_get_import_data',
					demo_name: demo_name,
					security: azeenDemos.azeen_import_data_nonce
				},
			...
			$.ajax( {
				...
				data: {
					action : 'azeen_ajax_get_demo_data',
					demo_name: demo_name,
					demo_data_nonce: azeenDemos.demo_data_nonce
				},
		...
		runPopup: function( data ) {
			...
			$( '#fwp-demo-import-form' ).submit( function( e ) {
				...
				// Vars
				var demo 	= $( this ).find( '[name="azeen_import_demo"]' ).val(),
					nonce 	= $( this ).find( '[name="azeen_import_demo_data_nonce"]' ).val(),
					contentToImport = [];
				...
				that.importContent( {
					...
					isXML: $( '#azeen_import_xml' ).is( ':checked' )
				} );
			} );
		...
		importContent: function( importData ) {
			var that = this,
				...
				timerStart = Date.now(),
				ajaxData = {
					azeen_import_demo: importData.demo,
					azeen_import_demo_data_nonce: importData.nonce
				};
			...
			if ( importData.contentToImport.length === 0 ) {
				...
				$.ajax( {
					...
					type: 'post',
					data: {
						action: 'azeen_after_import',
						azeen_import_demo: importData.demo,
						azeen_import_demo_data_nonce: importData.nonce,
						azeen_import_is_xml: importData.isXML
					},
					complete: function( data ) {}
				} );
				...
				complete: function( data ) {
					...
					if ( data.status === 500 || data.status === 502 || data.status === 503 ) {
						$( '.fwp-importing' )
							...
							.text( azeenDemos.content_importing_error + ' '+ data.status );
			...
			importingLimit = setTimeout( function() {
				...
				$( '.fwp-importing' )
					.addClass( 'fwp-importing-failed' )
					.removeClass( 'fwp-importing' )
					.text( azeenDemos.content_importing_error );
			}, 15 * 60 * 1000 );
		...
		activatePlugins: function( e ) {
			...
			$button.addClass( 'updating-message button-primary' ).html( azeenDemos.button_activating );

			$.ajax( {
				url: azeenDemos.ajaxurl,
				type: 'POST',
				data: {
					action : 'azeen_ajax_required_plugins_activate',
					init   : $init,
				},
		...
		installSuccess: function( e, response ) {
			...
			$message.removeClass( 'install-now installed button-disabled updated-message' )
				...
				.html( azeenDemos.button_activating );
